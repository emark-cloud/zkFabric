...
- **Frontend UX Polish (2026-04-12)**: Toast notifications, step indicators, proof substep progress, confirmation dialogs, empty states, CSS-only tooltips, landing page animations. ProofBuilder rewritten with natural-language dropdowns. CredentialCard shows human-readable slot values. VaultDashboard scope replaced with access requirement. User-friendly error messages across all pages. 6 new components: Toast, StepIndicator, ConfirmDialog, ProofContext, EmptyState, Tooltip. Landing page redesigned with problem/solution/use-cases sections for general audiences.
- **Deployment (2026-04-12)**: Frontend deployed to Vercel (`zk-fabric.vercel.app`), indexer and attestor deployed to Railway. SDK imports resolved via `@sdk/*` path alias for Vercel Turbopack compatibility.
